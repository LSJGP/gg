...
RUN_SIM = SIM_DIR / "run_sim.py"
VIZ_SIM = REPO_ROOT / "tools" / "viz_sim.py"
DEFAULT_METRICS = REPO_ROOT / "grading_mini" / "config" / "metrics_default.json"
DEFAULT_GRADING_BIN = (
    REPO_ROOT / "grading_mini" / "bazel-bin" / "src" / "entry" / "grading_main"
)
DEFAULT_SIM_RUNNER_HINT = "bazel-bin/cpp/sim_runner (build: cd sim && bazel build //cpp:sim_runner)"
...
def run_one_scenario(
    ...
    log_dir = Path(cfg.output_log_dir or (REPO_ROOT / "output" / "log"))
    report_dir = Path(cfg.output_report_dir or (REPO_ROOT / "output" / "report"))
    viz_dir = Path(cfg.output_viz_dir or (REPO_ROOT / "output" / "viz"))
    ...
    if cfg.run_grading:
        ...
        if not grading_bin.is_file():
            raise FileNotFoundError(
                ...
                "Build: cd grading_mini && bazel build //src/entry:grading_main"
            )
        sim_cmd.extend(
            [
                "--grading-bin",
    ...
    t0 = time.time()
    result["sim_rc"] = _run(sim_cmd, SIM_DIR, log)
    ...
    if cfg.make_gif and result["sim_rc"] == 0 and sim_log.is_file():
        ...
        result["viz_rc"] = _run(viz_cmd, REPO_ROOT, log)
...
def run_batch(
    ...
    mpath = metrics_config_path or (
        REPO_ROOT / "output" / "batch" / f"metrics_{int(time.time())}.json"
    )
    if cfg.run_grading:
