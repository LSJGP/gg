...
def _fill_boundary_segments(segs: List[Dict], out) -> None:
    for b in segs:
        seg = out.add()
...
def _fill_lane_neighbors(neighbors: List[Dict], out) -> None:
    for n in neighbors:
        ln = out.add()
...
def _track_state_from_dict(st: Dict) -> scenario_pb2.TrackState:
...
def scene_to_scenario_meta(
    ...
    meta.scenario_index = scenario_index
...
def lane_graph_doc_to_static_map(doc: Dict[str, Any]) -> map_pb2.StaticMap:
    """Build StaticMap from lane_graph.json dict."""
    sm = map_pb2.StaticMap()
    sm.source = str(doc.get("source", ""))
    sm.scenario_id = str(doc.get("scenario_id", ""))
    wo = doc.get("world_offset", {})
    sm.world_offset.x = float(wo.get("x", 0.0))
    sm.world_offset.y = float(wo.get("y", 0.0))
    sm.world_offset.z = float(wo.get("z", 0.0))
    counts = doc.get("map_feature_counts", {})
    for k in (
        "lanes",
        "road_lines",
        "road_edges",
        ...
        if k in counts and hasattr(sm.map_feature_counts, k):
            setattr(sm.map_feature_counts, k, int(counts[k]))
    ...
    for cw in doc.get("crosswalks", []):
        c = sm.crosswalks.add()
        c.id = int(cw["id"])
        for pt in cw.get("polygon", []):
            c.polygon.append(_vec3_from_list(pt))

    for ss in doc.get("stop_signs", []):
        s = sm.stop_signs.add()
        s.id = int(ss["id"])
        pos = ss.get("position", {})
        s.position.x = float(pos.get("x", 0.0))
    ...
    for dw in doc.get("driveways", []):
        d = sm.driveways.add()
        d.id = int(dw["id"])
        for pt in dw.get("polygon", []):
            d.polygon.append(_vec3_from_list(pt))

    for sb in doc.get("speed_bumps", []):
        b = sm.speed_bumps.add()
        b.id = int(sb["id"])
        for pt in sb.get("polygon", []):
            b.polygon.append(_vec3_from_list(pt))
...
def dynamic_objects_from_json_doc(doc: Dict[str, Any]) -> scenario_pb2.DynamicObjects:
    """Build DynamicObjects from split script JSON dict."""
    dyn = scenario_pb2.DynamicObjects()
