#!/usr/bin/env python3
"""Split dynamic_objects.json or .pb into per-frame files for stream loading.

Output layout under <scenario_dir>/dynamic_objects/:
  header.json / header.pb       metadata + track list without states
  sdc_states.json / .pb         SDC track with full states
  frames/NNNNN.json / .pb       per native timestamp, non-SDC states only
"""
...
def _load_dynamic_doc(scenario_dir: Path) -> Dict[str, Any]:
    ...
    if pb.is_file():
        from hyw_proto_convert import dynamic_objects_from_json_doc
        ...
        return {
            ...
            "world_offset": {
                "x": dyn.world_offset.x,
                "y": dyn.world_offset.y,
                "z": dyn.world_offset.z,
            },
    ...
    if js.is_file():
        with js.open(encoding="utf-8") as f:
            return json.load(f)
    raise FileNotFoundError(f"missing {pb} or {js}")


def _write_json_frames(
    out_base: Path,
    header: Dict[str, Any],
    sdc_track: Dict[str, Any] | None,
...
def split_dynamic_objects(scenario_dir: Path, fmt: str = "json") -> None:
    ...
    print(
        f"[split] {scenario_dir.name}: {len(timestamps)} frames, "
        f"{len(track_meta)} tracks -> {out_base} (format={fmt})"
    )
